...
```bash
export LED_SERIAL_PORT="COM5"   # your COM port / 你的串口号
export LED_COUNT=144            # your LED count / 你的 LED 数量
```
...
macOS 上核心逻辑完全相同，架构反而更简单——无需 WSL/Windows 分离，Daemon 和客户端在同一台机器上运行。

### What to change / 需要修改的地方
...
# Remove Windows-specific LOCALAPPDATA path
# 移除 os.environ.get("LOCALAPPDATA", ...) 逻辑
```
...
# Start daemon (no Start-Process needed)
python3 ~/led_controller/led_daemon.py &

# Kill daemon
kill $(cat /tmp/led_daemon.pid) 2>/dev/null

# Kill LED software (replace Skydimo with your app)
pkill -x "YourLEDApp"
...
**Serial port permissions** on macOS: you may need to run the daemon with `sudo` the first time, or add your user to the `_dialout` group:

macOS 串口权限：首次运行可能需要 `sudo`，或将用户加入 `_dialout` 组：
